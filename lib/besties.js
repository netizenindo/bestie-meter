...
function textOf(profile) {
  return [profile.bio || '', ...(profile.posts || []), ...(profile.replyTexts || [])].join('\n');
}
...
function computeSignals(a, b) {
  ...
  // Balasan/komentar dari tab replies (A membalas postingan @B, dst).
  const repliesAtoB = (a.repliedTo && a.repliedTo[ub]) || 0;
  const repliesBtoA = (b.repliedTo && b.repliedTo[ua]) || 0;
  const crossReplies = repliesAtoB + repliesBtoA;

  // Interaksi total = mention + balasan (per arah).
  const interAtoB = mentionsAtoB + repliesAtoB;
  const interBtoA = mentionsBtoA + repliesBtoA;
  const crossInteractions = interAtoB + interBtoA;
  const mutual = interAtoB > 0 && interBtoA > 0;
  ...
  // ---- meter: Saling Sapa (interaksi: mention + balasan) ----
  const salingSapa = clamp(crossInteractions * 10 + (mutual ? 20 : 0));
  ...
  const interaction = mutual
    ? Math.min(55, 25 + crossInteractions * 4)
    : Math.min(30, crossInteractions * 6);
  const vibe = sefrekuensi * 0.45; // 0-45
  const score = clamp(interaction + vibe);

  // ---- baris bukti (prioritas: interaksi, hashtag, jam aktif) ----
  const bits = [];
  if (crossInteractions > 0) bits.push('saling sapa ' + crossInteractions + '\u00D7');
  else bits.push('belum pernah saling sapa');
  ...
  return {
    score, mutual,
    crossInteractions, interAtoB, interBtoA,
    crossMentions, mentionsAtoB, mentionsBtoA,
    crossReplies, repliesAtoB, repliesBtoA,
