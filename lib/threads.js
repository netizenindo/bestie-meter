...
async function fetchProfile(username) {
  ...
  const mode = (process.env.THREADS_PROVIDER || 'direct').toLowerCase();
  const hasKey = !!process.env.SCRAPECREATORS_API_KEY;

  if (mode === 'scrapecreators' && hasKey) return viaScrapeCreators(u);
  if (mode === 'auto') return viaAuto(u, hasKey);
  return viaDirect(u);
}

// Profil "tipis" = nggak ada postingan & balasan sama sekali (kemungkinan keblok).
function isThin(p) {
  return !p || (!(p.posts && p.posts.length) && !(p.replyTexts && p.replyTexts.length));
}

// Coba direct dulu; tambal dari ScrapeCreators kalau direct keblok atau tanpa foto.
async function viaAuto(u, hasKey) {
  let direct = null, dErr = null;
  try { direct = await viaDirect(u); } catch (e) { dErr = e; }

  const needProvider = hasKey && (isThin(direct) || (direct && !direct.avatar));
  let sc = null;
  if (needProvider) { try { sc = await viaScrapeCreators(u); } catch (_) {} }

  if (!direct && !sc) throw dErr || new Error('Profil tidak terbaca dari sumber mana pun.');
  if (direct && !sc) return direct;
  if (!direct && sc) return sc;

  // Gabung: postingan/interaksi dari direct, foto/stat dari yang tersedia.
  const num = (a, b) => (a != null ? a : b);
  const arr = (a, b) => (a && a.length ? a : b);
  const obj = (a, b) => (a && Object.keys(a).length ? a : b);
  return {
    username: u,
    displayName: direct.displayName || sc.displayName || u,
    bio: direct.bio || sc.bio || '',
    avatar: direct.avatar || sc.avatar || null,
    stats: {
      followers: num(direct.stats && direct.stats.followers, sc.stats && sc.stats.followers),
      following: num(direct.stats && direct.stats.following, sc.stats && sc.stats.following)
    },
    posts: arr(direct.posts, sc.posts) || [],
    postTimes: arr(direct.postTimes, sc.postTimes) || [],
    replyTexts: arr(direct.replyTexts, sc.replyTexts) || [],
    repliedTo: obj(direct.repliedTo, sc.repliedTo) || {}
  };
}
...
function findUserByName(blocks, username) {
  for (const b of blocks) {
    ...
    while (stack.length) {
      ...
      if (!Array.isArray(cur) && typeof cur.username === 'string' &&
          cur.username.toLowerCase() === username &&
          (cur.profile_pic_url_hd || cur.profile_pic_url || 'follower_count' in cur)) {
        return cur;
      }
...
async function viaDirect(u) {
  ...
  // Utamakan ambil dari objek user milik PEMILIK akun (username cocok), biar
  // foto/bio/nama nggak ketuker sama akun lain di payload.
  const owner = findUserByName(blocks, u);
  if (owner) {
    followers = toNum(owner.follower_count);
    following = toNum(owner.following_count);
    bio = str(owner.biography);
    fullName = str(owner.full_name);
    avatar = str(owner.profile_pic_url_hd) || str(owner.profile_pic_url);
    if (owner.is_private === true || owner.text_post_app_is_private === true) isPrivate = true;
  }
